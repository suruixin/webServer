...
import { Login, RefreshToken } from '../config/intercept'
...
export default class Router {
   ...
   * @param res
   */
  public loginMethods (req: express.Request, res: express.Response): Promise<boolean> {
    return Login(req, res).then((data: TokenType | boolean) => {
      console.log(data)
      if (typeof data === 'boolean') {
        return Promise.resolve(false)
      } else {
        this.tokens = data.token;
        let sessions: any = req.session;
        sessions.auth = data;
  ...
  public setToken (req: express.Request): void {
    let session: any = (req.session || {});
    let token: string = (session.auth || {}).token;
    req.headers.authorization = token
  }

  public async refreshToken (req: express.Request): Promise<boolean> {
    let session: any = (req.session || {});
    let auth: TokenType = session.auth
    if (auth.validTime < (new Date().getTime() - conf.refreshToken.expiration)) {
      return Promise.resolve(true)
    } else {
      let Token: TokenType = await RefreshToken(auth.refresh);
